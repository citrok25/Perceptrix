...
import com.google.plus.samples.photohunt.custom.NetClientGet;
...
import com.perceptrix.databasing.InsertNewUser;
...
public class ConnectServlet extends JsonRestServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) {
	  System.out.println("\nIN THE DO POST OF CONNECT SERVLET\n");
    ...
    try {
      ...
      System.out.println("calling saveTokenForUser ");
      System.out.println("GoogleID: "+tokenGoogleUserId);
      
      User user = saveTokenForUser(tokenGoogleUserId, credential);
      
      
      
      //calling the NetClientGet class for REST call to get the email
      String email_id = com.google.plus.samples.photohunt.custom.NetClientGet.makeRequest(accessToken.access_token);
      
      System.out.println("email: "+email_id+"\nreturned completing saveTokenForUser routine");
      user.setEmail(email_id);
      
      databasing(user);
      createSession(req, user);
      //done databasing.
      //forward to _home.jsp
      
//      RequestDispatcher rd  =  req.getRequestDispatcher("_home.jsp");
//      try{
//    	  rd.forward(req, resp);
//      }
//      catch(Exception e){
...
//      }
      ...
      //System.out.println("session :"+session.getAttribute(CURRENT_USER_SESSION_KEY));
    ...
    System.out.println("Something The last line of Connect Servlet");
  }

  
  private boolean databasing(User user){
	  com.perceptrix.databasing.InsertNewUser inu = new InsertNewUser();
	  return inu.insertNewUser(user);
  }
  
  private void createSession(HttpServletRequest req, User user){
	  	HttpSession session = req.getSession();
        session.setAttribute("display_name",user.getGoogleDisplayName());
        session.setAttribute("email_id", user.getEmail());
        session.setAttribute("guid", user.getGoogleUserId());
        session.setAttribute("image_url", user.getGooglePublicProfilePhotoUrl());
        session.setAttribute("google_plus_url", user.getGooglePublicProfileUrl());
  ...
  private User saveTokenForUser(String tokenGoogleUserId,
    ...
    if (user == null) {
      ...
      //System.out.println("Before email manipulation code");
      //Getting Person's all Emails
      /*
      List<Person.Emails> emails = profile.getEmails();
      
      //iterating
      for(Person.Emails e: emails){
    	  if(e.getPrimary()){
    		  System.out.println("Primary: "+e.getValue());
    	  }
    	  else
    		  System.out.println("Primary: "+e.getValue());
      }*/
      ...
      System.out.println(//"EmailId: "+user.getEmail()+"\n"+
    		  ...
    		  "DisplayName: "+user.getGoogleDisplayName()+"\n"+
    		  "GUserId: "+user.getGoogleUserId()+"\n"
